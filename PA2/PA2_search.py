...
def UCSearch(start, goal, barrier):
    ...
    while len(fringe) > 0:
        ...
        # Otherwise (robot has not reached the goal)
        else:

            expandedNodes.add(curNode)  # marking the expansion of the current node

            # Going through each neighbor of the current node
            for neighbor in get_neighbors(curNode):

                # Checking if a neighbor has examined
                if neighbor not in fringe and neighbor not in expandedNodes:

                    fringe.add(neighbor)  # adding a neighbor to the fringe
                    parent[neighbor] = curNode  # updating the parent dictionary for retracing the path

                    G[neighbor] = move_cost(neighbor, barrier) + G[curNode]  # calculating a path cost of a neighbor

                # Otherwise (neighbor has already examined)
                else:

                    # Updating if a current path cost of a neighbor is less than the examined cost of this neighbor
                    if move_cost(neighbor, barrier) + G[curNode] < G[neighbor]:

                        G[neighbor] = move_cost(neighbor, barrier) + G[curNode]
        ...
        # Otherwise (robot has not reached the goal)
            ...
            for neighbor in get_neighbors(curNode):
                ...
                if neighbor not in fringe and neighbor not in expandedNodes:
                    ...
                    G[neighbor] = move_cost(neighbor, barrier) + G[curNode]  # calculating a path cost of a neighbor
                    F[neighbor] = heuristic(neighbor, goal) + G[neighbor]  # calculating a total cost of a neighbor

                # Otherwise (neighbor has already examined)
                else:

                    # Updating if current path cost of a neighbor is less than the examined cost of this neighbor
                    if move_cost(neighbor, barrier) + G[curNode] < G[neighbor]:

                        G[neighbor] = move_cost(neighbor, barrier) + G[curNode]
                        F[neighbor] = heuristic(neighbor, goal) + G[neighbor]
                        parent[neighbor] = curNode
